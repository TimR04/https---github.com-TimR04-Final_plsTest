...
def create_user_table():
    ...
    ensuring that each 'username' is unique by applying a UNIQUE constraint.

    Parameters:
    - None

    Returns:
    - None: The function performs the operation without returning any value.

    Tests:
    - Test 1: Table Creation Success
      - Input: The database is empty, and the `create_user_table()` function is called.
      - Expected Outcome: The 'users' table is successfully created, a log message is recorded, 
        and no errors are raised.

    - Test 2: Table Already Exists
      - Input: The 'users' table already exists, and the `create_user_table()` function is called.
      - Expected Outcome: No errors are raised, and the existing table remains unchanged. A log 
